...
fn main() -> Result<()> {
    ...
    match (args.next().as_deref(), args.next(), args.next(), args.next()) {
        ...
        (Some("exe"), Some(exe_path), Some(ico_path), None) => {
            ...
            println!("Extracted main icon from {} to {}", exe_path, ico_path);
        }
        ...
        (Some("dll"), Some(dll_path), Some(ico_dir), None) => {
            ...
            for (i, icon) in icons.iter().enumerate() {
                ...
                out_path.push(format!("{}.png", i));
                std::fs::write(&out_path, icon)
                    .with_context(|| format!("Failed to write icon file: {out_path:?}"))?;
            }
            println!("Extracted {} icons from {} to {}", icons.len(), dll_path, ico_dir);
        }
        ...
        (Some("bin"), Some(bin_path), Some(id), Some(ico_path)) => {
            ...
            std::fs::write(&ico_path, ico)
                .with_context(|| format!("Failed to write icon file: {ico_path}"))?;
            println!("Extracted icon {} from {} to {}", id, bin_path, ico_path);
        }
        ...
        (Some("bin"), Some(bin_path), Some(ico_dir), None) => {
            ...
            println!("Extracted {} icons from {} to {}", icos.len(), bin_path, ico_dir);
        }
        _ => {
            println!("Usage:");
            println!("  exeico exe <exe_path> <ico_path>         # Extract main icon from exe to .ico");
            println!("  exeico dll <dll_path> <ico_dir>          # Extract all icons from dll to .png");
            println!("  exeico bin <bin_path> <ico_id> <ico_path> # Extract icon by id from binary to .ico");
            println!("  exeico bin <bin_path> <ico_dir>           # Extract all icons from binary to .ico");
        }
    }
    Ok(())
}
